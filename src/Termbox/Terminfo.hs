...
type TerminfoResult = Either TerminfoError (U.Vector Word8)

loadTerminfo :: IO TerminfoResult
loadTerminfo = do
  term <- lookupEnv "TERM"
  case term of
    Nothing -> return . Left $ TermNotSet
    Just t  -> do
      terminfo <- tiTryLoad "TERMINFO" tiTryPath
      home     <- tiTryLoad "HOME" $ \p -> tiTryPath (p <> "/.terminfo")
      case terminfo <|> home of
        Left e ->  return . Left $ e
        Right c -> c

tiTryLoad :: String
          -> (String -> IO TerminfoResult)
          -> IO (Either TerminfoError (IO TerminfoResult))
tiTryLoad envVar cont = do
  varSet <- lookupEnv envVar
  case varSet of
    Nothing -> return . Left . EnvVarNotSet $ envVar
    Just c  -> Right <$> return (cont c)

tiTryPath :: String -> IO TerminfoResult
tiTryPath _ = return . Left $ TermNotSet
...
tiFuncs :: U.Vector Word16
tiFuncs = U.fromList [28, 40, 16, 13, 5, 39, 36, 27, 26, 34, 89, 88]

--------------------------------------------------------------------------------
tiKeys :: U.Vector Word16
tiKeys = U.fromList [ 66, 68, 69, 70, 71, 72, 73, 74, 75, 67 , 216
         , 217, 77, 59, 76, 164, 82, 81, 87, 61, 79, 83
         ]
